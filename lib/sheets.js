...
async function ensureSurveyResponseTab(sheets, spreadsheetId, slug, questions) {
  ...
  if (!sheetTitles.includes(slug)) {
    // Create sheet and grab its ID for formatting
    ...
    // Format: bold + wrap header, freeze row 1, wrap all data cells
    await sheets.spreadsheets.batchUpdate({
      spreadsheetId,
      requestBody: {
        requests: [
          {
            repeatCell: {
              ...
              fields: 'userEnteredFormat(wrapStrategy,textFormat,backgroundColor)',
            },
          },
          {
            repeatCell: {
...
export async function appendResponse(slug, questions, answers) {
  ...
  const dataRow = [
    ...
    ...questions.map(q => {
      ...
      // Resolve option val IDs → human-readable labels
      if (q.type === 'radio') {
